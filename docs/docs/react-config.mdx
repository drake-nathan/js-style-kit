...
React Fast Refresh validation is **disabled by default**. It should be enabled for SPAs like Vite. It should be left disabled for Next.js, Remix, and React Router projects (these frameworks handle their own refresh logic). To enable it:
...
JS Style Kit supports multiple React frameworks with automatic configuration:

### Next.js

For Next.js projects, use the `framework` option:
...
This will:
- Add `.next` to the list of ignored directories
- Include Next.js-specific ESLint rules
- Disable React Refresh (Next.js has its own Fast Refresh)
...
This will:
- Enable React Refresh validation by default
- Exclude Next.js-specific rules
...
This will:
- Disable React Refresh (Remix handles its own refresh logic)
- Exclude Next.js-specific rules
...
This will:
- Disable React Refresh (React Router handles its own refresh logic)
- Exclude Next.js-specific rules

### Override Framework Defaults

You can override the framework-based React Refresh behavior:
...
export default eslintConfig({
  react: {
    framework: "remix",
    reactRefresh: true, // Explicitly enable even though Remix is selected
  },
});
```
