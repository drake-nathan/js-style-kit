...
[React Compiler](https://react.dev/learn/react-compiler) is a compiler that optimizes React components for better performance. JS Style Kit includes support for React Compiler through the `eslint-plugin-react-hooks` plugin. It calls out places where you are violating the rules of hooks, and the compiler will not work.
